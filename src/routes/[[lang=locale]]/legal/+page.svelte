<script>
  ...
  import { getLegalNavItems } from '$lib/data/legalPages/index.js';
  import { t, localePath } from '$lib/i18n';

  const localeStore = getContext('locale');
  $: locale = $localeStore;

  $: navItems = getLegalNavItems(locale);
</script>

<SEO
  title={t(locale, 'legal.meta_title')}
  description={t(locale, 'legal.meta_desc')}
  {locale}
/>

<div class="legal-index">
  <div class="legal-index-header animate-on-scroll" use:scrollReveal>
    <h1 class="section-title">{t(locale, 'legal.page_title')}</h1>
    <p class="section-subtitle">
      {t(locale, 'legal.page_subtitle')}
    </p>
  </div>
  ...
  <div class="legal-cards">
    {#each navItems as item, i}
      <a
        href={localePath(locale, `/legal/${item.slug}`)}
        class="legal-card animate-on-scroll"
        use:scrollReveal
      ...
      >
        ...
        <h3>{item.label}</h3>
        <svg class="legal-card-arrow" width="16" height="16" viewBox="0 0 16 16" fill="none">
...
<style>
  ...
  .legal-index-header .section-subtitle {
    margin-top: 0.5rem;
  }
